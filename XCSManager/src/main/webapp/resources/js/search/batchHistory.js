...
$(function() {
	...
	$('#search').on('click', function() {
		...
		if(selectDate == "historyDate")
			json.historyDate = "historyDate";
	...
	list : function(json) {
		let success = function(result) {
			...
			if (result.msg != null && result.msg != "undefined" && result.msg != ""){
				...
				for (let i = 0; i < result.batchHistoryList.length; i++){
					let html = "";
					
					html += '<tr><td nowrap>' + blankToNull(result.batchHistoryList[i].historyId) + '</td>';
					html += '<td nowrap>' + blankToNull(result.batchHistoryList[i].historyDate) + '</td>';
					html += '<td nowrap>' + blankToNull(result.batchHistoryList[i].classification) + '</td>';
					html += '<td nowrap>' + blankToNull(result.batchHistoryList[i].processCount.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")) + '</td>';
					html += '<td nowrap>' + blankToNull(result.batchHistoryList[i].successCount.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")) + '</td>';
					
					$('#batchHistoryTable tbody').append(html);
				}
				
				if (result.batchHistoryCount != null && result.batchHistoryCount != "undefined" && result.batchHistoryCount != "") {
					totalData = result.batchHistoryCount;
					$('#selectCount').text(result.batchHistoryCount.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ","));
				}
		...
		AjaxUtil.ajax(ctx + "/getBatchHistoryList", "POST", json, success, error);
	}
}; 
